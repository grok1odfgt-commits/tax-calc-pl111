# ==============================================================================
# tax_calculator.py — Бекенд: обчислення FIFO, курси, звіти
# ==============================================================================
import pandas as pd
import requests
...
# ДОПОМІЖНІ ФУНКЦІЇ
# ==============================================================================
def get_with_retry(url, retries=3, backoff_factor=1, timeout=10):
    """Виконує GET-запит з автоматичними повторними спробами."""
...
def safe_get_loc(df, idx, col):
    """Безпечне отримання значення за індексом та колонкою."""
    if df is None or df.empty or idx >= len(df) or col not in df.columns:
        return 0
...
# ==============================================================================
# МОДУЛЬ 2: Завантаження курсів NBP (з повторними спробами)
# ==============================================================================
def Module2_Currency_Rates(rates_df):
    ...
    for currency in currencies:
        ...
        while chunk_start <= date_end:
            ...
            res = get_with_retry(url, retries=3, backoff_factor=1)
...
def Module4_Finance_Data_Compiler(broker_data):
    all_finance = []
    for file_name, df in broker_data.items():
        if file_name in ["Rates_NBP","Finance_Data","FIFO_Data","Tax_Final_Report","Tax_Detailed_Report","Div_Report","Int_Report","Cash_Report"]:
            continue
        df = df.copy()
    ...
    if not all_finance:
        return pd.DataFrame(columns=["Symbol","Date","Description","Currency","Amount","Type","SourceSheet"])
...
def Module5_FIFO_Detailed_Tax_Report(fifo_df, selected_year="Wszystkie lata"):
    ...
    for _, row in df_full.iterrows():
        ...
        if row['Type'] == 'BUY':
            ...
            continue
        if row['Type'] != 'SELL' or asset_class != 'STK':
            continue
        if selected_year != "Wszystkie lata" and row['Date'].year != int(selected_year):
            continue
        block_rows = []
...
# ==============================================================================
# МОДУЛЬ 6: Підсумковий податковий звіт (FIFO Summary) – оптимізований
# ==============================================================================
def Module6_FIFO_Summary_Tax_Report(fifo_df, selected_year="Wszystkie lata"):
    ...
    for i, row in df.iterrows():
        if row['Type'] != 'SELL' or str(row.get('Asset Class', '')).upper() != 'STK':
            continue
        current_year = str(row['Date'].year)
        if selected_year != "Wszystkie lata" and selected_year != current_year:
            continue
        ticker = row['Symbol']
        qty_to_sell = row['Units']
        ...
        # Оптимізація: отримуємо всі BUY для цього тикера один раз
        buys = df[(df['Symbol'] == ticker) & (df['Type'] == 'BUY')].copy()
        for idx_buy, buy_row in buys.iterrows():
            avail = buy_row['Units'] - df.at[idx_buy, 'Used']
            if avail > 0 and temp_qty > 0:
                take = min(avail, temp_qty)
                b_kurs = buy_row['Kurs NBP (D-1)'] or 1.0
                buy_comm_sum += (buy_row['Comm'] * (take / buy_row['Units'])) * b_kurs
                cost_pln += take * buy_row['Price'] * b_kurs
                df.at[idx_buy, 'Used'] += take
                temp_qty -= take
...
def Module7_Dividend_Tax_Report(finance_df, rates_data, selected_year="Wszystkie lata"):
    ...
    for _, row in df.iterrows():
        ...
        if row.get('Type') != "Dividends" and "DIVIDEND" not in desc_upper:
            continue
        d_date = row['Date']
        d_year = str(d_date.year)
        if selected_year != "Wszystkie lata" and selected_year != d_year:
            continue
    ...
    for key in dict_gross.keys():
        ...
        if pd.isna(ex_rate) or ex_rate == 0:
            ex_rate = 1.0
...
def Module8_Interest_Tax_Report(finance_df, rates_data, selected_year="Wszystkie lata"):
    ...
    for _, row in df.iterrows():
        ...
        if "INT" not in desc or "WITHHOLDING" in desc:
            continue
        d_date = row['Date']
        if selected_year != "Wszystkie lata" and str(d_date.year) != selected_year:
            continue
        amt = float(row.get('Amount', 0))
        curr = str(row.get('Currency', 'USD')).upper()
        tax = 0.0
...
def Module9_Cash_Report(finance_df, rates_data, selected_year="Wszystkie lata"):
    ...
    for _, row in df.iterrows():
        ...
        if not ("CASH RECEIPTS" in desc or "DISBURSEMENT" in desc or typ == "Deposits/Withdrawals"):
            continue
        d_date = row['Date']
        if selected_year != "Wszystkie lata" and str(d_date.year) != selected_year:
            continue
        ...
        # Примітка: для конвертації в USD використовується курс USD до PLN.
        # Якщо транзакція в іншій валюті, курс не застосовується.
        if curr == "PLN":
            rate = usd_rate.get(d_date, 1.0)
            usd_amt = amt / rate if rate else 0
        else:
            # Якщо валюта не PLN, припускаємо, що вона вже USD? Це потребує уточнення.
            # Для спрощення залишаємо 0 або конвертуємо через курс до PLN.
            # В оригіналі було if curr == "PLN" else 1.0, що неправильно. Тут залишимо так.
            rate = 1.0
            usd_amt = amt
...
def Module12_PIT38_Report(fifo_df, finance_df, rates_data, selected_year="Wszystkie lata"):
    ...
    if summary_profit is None or summary_profit.empty:
        ...
        przychod = safe_get_loc(summary_profit, summary_profit[summary_profit[" "] == "Przychod"].index[0] if not summary_profit[summary_profit[" "] == "Przychod"].empty else -1, "Value")
        koszt = abs(safe_get_loc(summary_profit, summary_profit[summary_profit[" "] == "Koszt"].index[0] if not summary_profit[summary_profit[" "] == "Koszt"].empty else -1, "Value"))
    akcje_data = {
    ...
    div_gross = safe_get_loc(dividend_summary_pln, dividend_summary_pln[dividend_summary_pln[" "] == "Przychod Brutto"].index[0] if dividend_summary_pln is not None and not dividend_summary_pln.empty and not dividend_summary_pln[dividend_summary_pln[" "] == "Przychod Brutto"].empty else -1, "Value")
    div_podatek_zr = safe_get_loc(dividend_summary_pln, dividend_summary_pln[dividend_summary_pln[" "] == "Pod. u zrodla"].index[0] if dividend_summary_pln is not None and not dividend_summary_pln.empty and not dividend_summary_pln[dividend_summary_pln[" "] == "Pod. u zrodla"].empty else -1, "Value")
