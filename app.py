# ==============================================================================
# ==============================================================================
# КАЛЬКУЛЯТОР ПОДАТКІВ FIFO (Streamlit) — ПОВНИЙ КОД З УСІМА 12 МОДУЛЯМИ
# ==============================================================================
# ==============================================================================
...
def render_sidebar():
    with st.sidebar:
        ...
        uploaded_files = st.file_uploader("Завантажте CSV файли від брокера:", accept_multiple_files=True)
        if st.button("🔄 Оновити все"):
...
def render_Rates_NBP_Tab():
    ...
    if st.button("Завантажити курси з NBP"):
        st.session_state.rates_data = Module2_Currency_Rates(st.session_state.rates_data)
    st.dataframe(st.session_state.rates_data, use_container_width=True)
...
def render_FIFO_Data_Tab():
    ...
    if st.button("Скомпілювати дані (FIFO)"):
    ...
    if st.session_state.fifo_df is not None:
        st.dataframe(st.session_state.fifo_df, use_container_width=True)
    else:
        st.info("Натисніть кнопку вище")
...
def render_Finance_Data_Tab():
    ...
    if st.button("🔄 Скомпілювати Finance Data"):
        st.session_state.finance_df = Module4_Finance_Data_Compiler(st.session_state.broker_data)
        st.rerun()
    if st.session_state.get('finance_df') is not None and not st.session_state.finance_df.empty:
        st.dataframe(st.session_state.finance_df, use_container_width=True)

# ==============================================================================
# МОДУЛЬ 5 + render
# ==============================================================================
def Module5_FIFO_Detailed_Tax_Report(fifo_df, selected_year="Wszystkie"):
    ...
    for _, row in df_full.iterrows():
        ...
        if selected_year != "Wszystkie" and row['Date'].year != int(selected_year): continue
        block_rows = []
...
def render_Tax_Detailed_Report_Tab():
    ...
    years = ["Wszystkie"] + sorted([str(y) for y in st.session_state.fifo_df['Date'].dt.year.unique()])
    selected_year = st.selectbox("Оберіть рік:", years, key="year_select")
    if st.button("Сформувати детальний звіт"):
    ...
    if st.session_state.report_blocks:
        col1, col2 = st.columns([1.4, 1.6])
        with col1:
            st.markdown("**Podsumowanie sprzedaz**")
            st.dataframe(st.session_state.sales_summary.style.set_properties(**{'font-weight': 'bold'}), hide_index=True, use_container_width=False)
        with col2:
            ...
            def profit_style(row):
                ...
                return styles
            st.dataframe(st.session_state.profit_summary.style.apply(profit_style, axis=1).format({"Value": "{:,.7f}"}).set_properties(**{'font-weight': 'bold'}), hide_index=True, use_container_width=False)
        for block in st.session_state.report_blocks:
            if block.empty: continue
            def safe_format(x):
                if pd.isna(x) or isinstance(x, str): return x if isinstance(x, str) else ""
                return f"{float(x):,.7f}"
            styled = block.style.map(lambda v: 'color: #9c0006' if isinstance(v, (int, float)) and v < 0 else 'color: #006100', subset=['Przepływ [PLN]']).format({"Cena": safe_format, "Kwota": safe_format, "Prowizja": safe_format, "Jednostki": safe_format, "Przychod [PLN]": safe_format, "Koszt [PLN]": safe_format, "Przepływ [PLN]": safe_format, "Kurs NBP": safe_format})
...
# МОДУЛЬ 6 + render
# ==============================================================================
def Module6_FIFO_Summary_Tax_Report(fifo_df, selected_year="Wszystkie"):
    ...
    for i, row in df.iterrows():
        ...
        if selected_year != "Wszystkie" and selected_year != current_year: continue
        ticker = row['Symbol']
        qty_to_sell = row['Units']
...
def render_Tax_Summary_Report_Tab():
    ...
    years = ["Wszystkie"] + sorted([str(y) for y in st.session_state.fifo_df['Date'].dt.year.unique()])
    selected_year = st.selectbox("Оберіть рік:", years, key="summary_year_select")
    if st.button("🔄 Сформувати підсумковий звіт"):
        main_df, sales_sum, profit_sum = Module6_FIFO_Summary_Tax_Report(st.session_state.fifo_df, selected_year)
        st.session_state.summary_df = main_df
        st.session_state.summary_sales = sales_sum
        st.session_state.summary_profit = profit_sum
        st.rerun()
    if st.session_state.get('summary_df') is not None and not st.session_state.summary_df.empty:
        col1, col2 = st.columns([1.4, 1.6])
        with col1:
            st.markdown("**Podsumowanie sprzedaz**")
            st.dataframe(st.session_state.summary_sales.style.set_properties(**{'font-weight': 'bold'}), hide_index=True, use_container_width=False)
        with col2:
            st.markdown("**Ogolny profit [PLN]**")
            def profit_style(row):
                styles = [''] * 2
                if row[" "] == "Przeplyw":
                    styles = ['color: #006100'] * 2 if row["Value"] >= 0 else ['color: #9c0006'] * 2
                return styles
            st.dataframe(st.session_state.summary_profit.style.apply(profit_style, axis=1).format({"Value": "{:,.2f}"}).set_properties(**{'font-weight': 'bold'}), hide_index=True, use_container_width=False)
        st.markdown("**Детальна таблиця продажів**")
        styled = st.session_state.summary_df.style.format({"Przeplyw PLN": "{:,.2f}", "Data sprzedazy": lambda x: x.strftime('%Y-%m-%d')}).apply(lambda x: ['color: #006100' if v >= 0 else 'color: #9c0006' for v in x], subset=['Przeplyw PLN'])
        st.dataframe(styled, use_container_width=True, hide_index=True)
    else:
        st.info("Натисніть кнопку вище")

# ==============================================================================
# МОДУЛЬ 7 + render (дивіденди) — ВИПРАВЛЕНИЙ
# ==============================================================================
def Module7_Dividend_Tax_Report(finance_df, rates_data, selected_year="Wszystkie"):
    ...
    for _, row in df.iterrows():
        ...
        if selected_year != "Wszystkie" and selected_year != d_year: continue
    ...
    summary_val = pd.DataFrame({
        " ": ["Прихід брутто", "Податок у джерела", "Середня ставка у джерела", "Сума нетто"], 
        "Value": [total_gross, total_tax, avg_rate, total_gross - total_tax]
    })
    summary_pln = pd.DataFrame({
        " ": ["Przychod Brutto", "Pod. u zrodla", "Podatek w PL (19%)", "Doplata w PL", "Suma Netto"], 
        "Value": [total_pln_gross, total_pln_tax_source, total_pln_tax19, total_pln_topup, total_pln_gross - total_pln_tax_source - total_pln_topup]
    })
    return main_df, summary_val, summary_pln

def render_Tax_Dividend_Report_Tab():
    st.markdown("<h3 style='text-align: center;'>💰 Tax Dividend — податковий звіт по дивідендах</h3>", unsafe_allow_html=True)
    if st.session_state.get('finance_df') is None or st.session_state.finance_df.empty:
        st.info("Спочатку скомпілюйте Finance Data")
        return
    date_col = pd.to_datetime(st.session_state.finance_df['Date'], errors='coerce')
    years = ["Wszystkie"] + sorted([str(y) for y in date_col.dt.year.dropna().unique()])
    selected_year = st.selectbox("Оберіть рік:", years, key="dividend_year_select")
    if st.button("🔄 Сформувати звіт по дивідендах"):
        main_df, sum_val, sum_pln = Module7_Dividend_Tax_Report(st.session_state.finance_df, st.session_state.rates_data, selected_year)
        st.session_state.dividend_df = main_df
        st.session_state.dividend_summary_val = sum_val
        st.session_state.dividend_summary_pln = sum_pln
        st.rerun()
    if st.session_state.get('dividend_df') is not None and not st.session_state.dividend_df.empty:
        col1, col2 = st.columns(2)
        with col1:
            st.markdown("<h4 style='text-align: center;'>Підсумування в валюті</h4>", unsafe_allow_html=True)
            st.dataframe(
                st.session_state.dividend_summary_val.style
                    .set_properties(**{'font-weight': 'bold'})
                    .format({"Value": "{:,.2f}"})
                    .format({"Value": "{:.2%}"}, subset=pd.IndexSlice[2:3, "Value"]),
                hide_index=True, use_container_width=True
            )
        with col2:
            st.markdown("<h4 style='text-align: center;'>Підсумування (PLN)</h4>", unsafe_allow_html=True)
            def pln_style(row):
                styles = [''] * 2
                ...
                return styles
            st.dataframe(
                st.session_state.dividend_summary_pln.style
                    .apply(pln_style, axis=1)
                    .format({"Value": "{:,.2f}"})
                    .set_properties(**{'font-weight': 'bold'}),
                hide_index=True, use_container_width=True
            )
        st.markdown("<h4 style='text-align: center;'>Детальна таблиця дивідендів</h4>", unsafe_allow_html=True)
        styled = st.session_state.dividend_df.style.format({
            "Data": lambda x: x.strftime('%Y-%m-%d'), "Data NBP": lambda x: x.strftime('%Y-%m-%d'),
...
# ==============================================================================
# МОДУЛЬ 8 + render (відсотки)
# ==============================================================================
def Module8_Interest_Tax_Report(finance_df, rates_data, selected_year="Wszystkie"):
    if finance_df is None or finance_df.empty: return pd.DataFrame(), pd.DataFrame(), pd.DataFrame()
    df = finance_df.copy()
    df['Date'] = pd.to_datetime(df['Date']).dt.date
    report_rows = []
    ...
    for _, row in df.iterrows():
        ...
        d_date = row['Date']
        if selected_year != "Wszystkie" and str(d_date.year) != selected_year: continue
        amt = float(row.get('Amount', 0))
        curr = str(row.get('Currency', 'USD')).upper()
        tax = 0.0
...
def render_Tax_Interest_Report_Tab():
    ...
    date_col = pd.to_datetime(st.session_state.finance_df['Date'], errors='coerce')
    years = ["Wszystkie"] + sorted([str(y) for y in date_col.dt.year.dropna().unique()])
    selected_year = st.selectbox("Оберіть рік:", years, key="interest_year_select")
    if st.button("🔄 Сформувати звіт по відсотках"):
        main, sval, spln = Module8_Interest_Tax_Report(st.session_state.finance_df, st.session_state.rates_data, selected_year)
        st.session_state.interest_df = main
        st.session_state.interest_summary_val = sval
        st.session_state.interest_summary_pln = spln
        st.rerun()
    if st.session_state.get('interest_df') is not None and not st.session_state.interest_df.empty:
        col1, col2 = st.columns([1.3, 1.7])
        with col1: st.markdown("**Podsumowanie (VAL)**"); st.dataframe(st.session_state.interest_summary_val.style.set_properties(**{'font-weight': 'bold'}), hide_index=True)
        with col2: st.markdown("**Podsumowanie (PLN)**"); st.dataframe(st.session_state.interest_summary_pln.style.format({"Value": "{:,.2f}"}).set_properties(**{'font-weight': 'bold'}), hide_index=True)
        st.markdown("**Детальна таблиця відсотків**")
        st.dataframe(st.session_state.interest_df.style.format({"Data": lambda x: x.strftime('%Y-%m-%d'), "Data NBP": lambda x: x.strftime('%Y-%m-%d'), "Przychod": "{:,.2f}", "Pod. zrodlo": "{:,.2f}", "Netto": "{:,.2f}", "Stawka zr. %": "{:.2%}", "Kurs": "{:,.4f}", "Przychod (PLN)": "{:,.2f}", "Pod. zr. (PLN)": "{:,.2f}", "Pod. PL (19%)": "{:,.2f}", "Doplata (PLN)": "{:,.2f}", "Netto (PLN)": "{:,.2f}"}), use_container_width=True, hide_index=True)

# ==============================================================================
# МОДУЛЬ 9 + render (Cash)
# ==============================================================================
def Module9_Cash_Report(finance_df, rates_data, selected_year="Wszystkie"):
...
def render_Cash_Report_Tab():
    ...
    date_col = pd.to_datetime(st.session_state.finance_df['Date'], errors='coerce')
    years = ["Wszystkie"] + sorted([str(y) for y in date_col.dt.year.dropna().unique()])
    selected_year = st.selectbox("Оберіть рік:", years, key="cash_year_select")
    if st.button("🔄 Сформувати Cash Report"):
        main, summ = Module9_Cash_Report(st.session_state.finance_df, st.session_state.rates_data, selected_year)
        st.session_state.cash_df = main
        st.session_state.cash_summary = summ
        st.rerun()
    if st.session_state.get('cash_df') is not None and not st.session_state.cash_df.empty:
        st.dataframe(st.session_state.cash_df.style.format({"Data": lambda x: x.strftime('%Y-%m-%d'), "Kwota (PLN)": "{:,.2f}", "Kwota (USD)": "{:,.2f}", "Kurs (USD)": "{:,.4f}"}), use_container_width=True, hide_index=True)
        st.markdown("**Підсумки**")
        st.dataframe(st.session_state.cash_summary.style.format({"Value": "{:,.2f}"}).set_properties(**{'font-weight': 'bold'}), hide_index=True)

# ==============================================================================
# МОДУЛЬ 10 + render
# ==============================================================================
def Module10_Transactions_Report(fifo_df, selected_year="Wszystkie"):
...
def render_Transactions_Report_Tab():
    ...
    if st.session_state.get('fifo_df') is None or st.session_state.fifo_df.empty:
        st.info("Скомпілюйте FIFO Data спочатку")
        return
    years = ["Wszystkie"] + sorted([str(y) for y in st.session_state.fifo_df['Date'].dt.year.unique()])
    selected_year = st.selectbox("Оберіть рік:", years, key="trans_year_select")
    if st.button("🔄 Сформувати Transactions Report"):
        st.session_state.transactions_df = Module10_Transactions_Report(st.session_state.fifo_df, selected_year)
        st.rerun()
    if st.session_state.get('transactions_df') is not None and not st.session_state.transactions_df.empty:
        styled = st.session_state.transactions_df.style.format({
            "Data i Czas": lambda x: x.strftime('%Y-%m-%d %H:%M:%S'),
...
def render_Portfolio_Tab():
    ...
    if st.session_state.get('fifo_df') is None or st.session_state.fifo_df.empty:
        st.info("Скомпілюйте FIFO Data спочатку")
        return
    if st.button("🔄 Оновити портфель"):
        main, curr_percent, curr_value = Module11_Portfolio(st.session_state.fifo_df, st.session_state.rates_data)
        st.session_state.portfolio_df = main
    ...
    if st.session_state.get('portfolio_df') is not None and not st.session_state.portfolio_df.empty:
        ...
        with col1:
            ...
            st.dataframe(st.session_state.portfolio_currency_percent.style.format({"Udział %": "{:.2%}"}), hide_index=True)
        with col2:
            st.markdown("**Вартість за валютою (оригінальна)**")
            st.dataframe(st.session_state.portfolio_currency_value.style.format({"Wartosc": "{:,.2f}"}), hide_index=True)

# ==============================================================================
# МОДУЛЬ 12: PIT-38 REPORT — ТОЧНО ЯК У VBA (виправлено помилку + повернуто оригінальні назви)
# ==============================================================================
def Module12_PIT38_Report(summary_df, summary_profit, dividend_summary_pln):
    ...
    # === PIT-38 — AKCJE I KOSZTY (точно як у VBA) ===
    akcje_data = {
        "Komorka": ["C.20", "C.21", "C.22", "C.23", "C.26", "C.27", "C.28", "C.29", "D.31", "D.32", "D.33", "D.34", "D.35"],
        "Nazwa": [
            "Przychody wykazane w części D informacji PIT-8C",
            "Przychody wykazane w części D informacji PIT-8C",
            "Inne przychody / Przychod",
            "Inne przychody / Koszty uzyskania przychodow",
            "Razem (suma kwot z wierszy 1 do 2) / Przychod",
            "Razem (suma kwot z wierszy 1 do 2) / Koszty uzyskania przychodow",
            "Dochod (b-c)", "Strata (c-b)",
            "Podstawa obliczenia podatku (po zaokragleniu do pełnych złotych)",
            "Stawka podatku (należy podać w procentach)",
            "Podatek od dochodów, o których mowa w art. 30b ust. 1 ustawy",
        ...
        "Wartosc": [
            0, 0, przychod, koszt,
            przychod, koszt,
            max(0, przychod - koszt),
            max(0, koszt - przychod),
            round(max(0, przychod - koszt)),
            19,
            round(max(0, przychod - koszt) * 0.19),
            0,
            round(max(0, przychod - koszt) * 0.19)
        ]
    }
    ...
    # === PIT-38 — DYWIDENDY (точно як у VBA) ===
    div_gross = 0
    div_podatek_zr = 0
    ...
    dywidendy_data = {
        ...
        "Nazwa": [
            "Suma wypłat dywidend zagranicznych - podstawa opodatkowania (wiersz pomocniczy)",
            "Zryczałtowany podatek obliczony od przychodów (dochodów), o których mowa w art. 30a ust. 1 pkt 1–5 ustawy, uzyskanych poza granicami Rzeczypospolitej Polskiej (19%)",
            "Podatek zapłacony za granicą, o którym mowa w art. 30a ust. 9 ustawy (przeliczony na złote)",
            "Dokładna wartość podatku do dopłacenia (wiersz pomocniczy)",
            "Różnica między zryczałtowanym podatkiem a podatkiem zapłaconym za granicą (G.47 - G.48, po zaokrągleniu do pełnych złotych)"
        ],
    ...
    if summary_df is not None and not summary_df.empty and "Kraj emitenta" in summary_df.columns:
        zg = summary_df[
            summary_df["Kraj emitenta"].notna() &
            (summary_df["Kraj emitenta"] != "") &
            (summary_df["Kraj emitenta"] != "PL")
        ].copy()
        if not zg.empty:
...
# ==============================================================================
# ВІЗУАЛ МОДУЛЯ 12 (з блоком "PIT-38 - Podatek do zaplaty")
# ==============================================================================
def render_PIT38_Tab():
    st.markdown("<h3 style='text-align: center;'>📋 PIT-38 — підсумковий податковий звіт</h3>", unsafe_allow_html=True)
    
    if st.session_state.get('summary_df') is None or st.session_state.summary_df.empty:
        st.warning("Спочатку сформуйте підсумковий податковий звіт (Tax_Summary_Report)")
        return
    
    years = ["Wszystkie"] + sorted([str(y) for y in st.session_state.fifo_df['Date'].dt.year.unique()]) if st.session_state.fifo_df is not None else ["Wszystkie"]
    selected_year = st.selectbox("Wybierz rok:", years, key="pit38_year_select")
    
    if st.button("🔄 Сформувати PIT-38"):
        df_akcje, df_dyw, df_zg = Module12_PIT38_Report(
            st.session_state.summary_df,
            st.session_state.summary_profit,
            st.session_state.dividend_summary_pln
        )
        st.session_state.pit38_akcje = df_akcje
        st.session_state.pit38_dywidendy = df_dyw
        st.session_state.pit38_zg = df_zg
        st.rerun()
    
    if st.session_state.get('pit38_akcje') is not None:
        col1, col2 = st.columns(2)
        
        with col1:
            st.markdown("**PIT-38 - Akcje i Koszty**")
            st.dataframe(st.session_state.pit38_akcje.style.format({"Wartosc": "{:,.2f}"}).set_properties(**{'font-weight': 'bold'}), hide_index=True, use_container_width=True)
        
        with col2:
            st.markdown("**PIT-38 - Dywidendy**")
            st.dataframe(st.session_state.pit38_dywidendy.style.format({"Wartosc": "{:,.2f}"}).set_properties(**{'font-weight': 'bold'}), hide_index=True, use_container_width=True)
        
        # === НОВИЙ БЛОК: PIT-38 - Podatek do zaplaty ===
        ...
        st.dataframe(podatek_df.style.format({"Wartosc": "{:,.2f}"}).set_properties(**{'font-weight': 'bold'}), hide_index=True, use_container_width=True)
        
        st.markdown("**PIT/ZG — Zagraniczne przychody**")
        st.dataframe(st.session_state.pit38_zg.style.format({"Inne przychody, w tym uzyskane za granicą - Dochod": "{:,.2f}", "Podatek od innych przychodów zapłacony za granicą": "{:,.2f}"}), hide_index=True, use_container_width=True)
        ...
        if st.button("📥 Завантажити Excel PIT-38.xlsx"):
            ...
            output.seek(0)
            st.download_button(label="⬇️ Завантажити PIT-38.xlsx", data=output, file_name=f"PIT-38_{selected_year}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

# ==============================================================================
# ГОЛОВНИЙ ОРКЕСТРАТОР ВКЛАДОК
# ==============================================================================
    ...
    for i, name in enumerate(tabs_names):
        with tabs[i]:
            if name in st.session_state.broker_data:
                ...
                st.dataframe(st.session_state.broker_data.get(name), use_container_width=True)
...
if uploaded_files and st.session_state.broker_data is None:
    st.session_state.broker_data, st.session_state.rates_data = Module1_Data_Import(uploaded_files)
if st.session_state.broker_data:
    render_main_tabs()
else:
    st.info("Будь ласка, завантажте CSV файли брокера.")